# -*- coding: utf-8 -*-
"""Project420Final.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1y2dvF3Q1r-fcPljKwwwlkiEnLBdeDAHj
"""
...
def segmentation_index_row(seg,contours):
  ...
  for i in range(len(row_list)):
    if not (i == 0 or i == len(row_list)-1):
      if row_list[i] < (minimum_count_row*1.5):
        if row_list[i+1] > row_list[i]:
          if not row_list[i-1] in credict_point_row:
...
def segmentation_index_col(seg,credict_point_row,contours):
  ...
  for i in range(len(col_list)):
    if not (i == 0 or i == len(col_list)-1):
      if col_list[i] < (minimum_count*1.5):
        if col_list[i+1] > col_list[i]:
          if not col_list[i-1] in credict_point_col:
            credict_point_col.append(i)
          else:
            credict_point_col.remove(i-1)
            credict_point_col.append(i)
  credict_point_col.sort()
  return credict_point_col,digits
...
#find the segmentation index for license plate
    ...
    credict_point_col,digit = segmentation_index_col(seg_seg,credict_point_row,contour)
...
#segmented plate
    ...
    for i in range(len(seg_list)):
      ...
      cv2.imwrite(seg_path,seg_list[i])
    count+=1

path = '/content/drive/My Drive/Colab Notebooks/car.png'
find_license_plate(path)